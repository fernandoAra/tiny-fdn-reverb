...
| Paper-like optimizer settings | `eval/difffdn/optimize_householder.py` | Added paper-oriented defaults (`fs=48k`, `M=480000`, `batch=2000`, `epochs`) with **with-decay** default training mode (`gamma` from RT60), and optional lossless-core mode for ablations | Dal Santo EURASIP 2025 training framing + diff-fdn-colorless commit above |
...
| Echo-density proxy metric | `eval/scripts/compare_fixed_vs_diff.py` | Added short-time envelope peak/event-rate proxy (events/s), summarized in 50–300 ms window; intended as a practical temporal-density indicator | Project-specific evaluation metric (not a direct paper equation) |
...
- Temporal-density reporting uses two offline proxies: RMS-normalized excess kurtosis and envelope peak event-rate (events/s).
- These metrics are included to improve interpretability/reproducibility of tiny-FDN comparisons; they are proxies, not direct perceptual ground truth.
