...
plugins: libs
	$(MAKE) all -C plugins/tiny-fdn-reverb
...
clean:
	...
	$(MAKE) clean -C plugins/tiny-fdn-reverb
	rm -rf bin build

install: all
	$(MAKE) install -C plugins/tiny-fdn-reverb

install-user: all
	$(MAKE) install-user -C plugins/tiny-fdn-reverb
